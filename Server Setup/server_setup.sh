...
# Execute commands through a login shell so apt/ufw/systemctl paths behave consistently.
# This is more robust than raw eval for automation.
execute() {
    local cmd="$1"
    local fatal="${2:-true}"
    ...
    if ! bash -lc "$cmd"; then
        if [ "$fatal" = "true" ]; then
            handle_error "Failed to execute: $cmd"
        else
...
# Ensure locale support is installed before generating/updating locales.
log "Installing locale support package..."
execute "apt install -y locales"

# Check whether TPM2 auto-unlock is already configured on the current root device.
AUTO_UNLOCK_ENABLED=false
ROOT_LUKS_DEVICE=""

if command -v findmnt >/dev/null 2>&1; then
    ROOT_LUKS_DEVICE=$(findmnt -no SOURCE / 2>/dev/null || true)
fi

if [ -n "$ROOT_LUKS_DEVICE" ] && command -v systemd-cryptenroll >/dev/null 2>&1; then
    if systemd-cryptenroll "$ROOT_LUKS_DEVICE" 2>/dev/null | grep -Eq 'tpm2:\s*yes|^\s*1\s+tpm2'; then
        AUTO_UNLOCK_ENABLED=true
    fi
fi

if [ "$AUTO_UNLOCK_ENABLED" = true ]; then
    log "TPM2 auto-unlock already appears configured; skipping auto-unlock setup."
else
    log "TPM2 auto-unlock not detected on $ROOT_LUKS_DEVICE; standard setup will continue."
fi
...
# Install the base toolchain and hardening packages.
# Note: chrony replaces the older ntp package on modern Ubuntu releases.
# apt-transport-https is no longer needed as a separate package on current apt.
...
# Configure TPM2 auto-unlock (systemd-based) only when it is not already enabled.
if [ "$AUTO_UNLOCK_ENABLED" = false ]; then
    log "Setting up TPM2 auto-unlock with systemd..."
    execute "apt install -y tpm2-tools cryptsetup-initramfs" false

    ROOT_LUKS_DEVICE="$(findmnt -no SOURCE / 2>/dev/null || true)"
    if [ -n "$ROOT_LUKS_DEVICE" ] && cryptsetup isLuks "$ROOT_LUKS_DEVICE" 2>/dev/null; then
        execute "systemd-cryptenroll --tpm2-device=auto --tpm2-pcrs=7+11+14 '$ROOT_LUKS_DEVICE'" false
        execute "update-initramfs -u" false
        log "TPM2 auto-unlock setup attempted. Reboot to verify the new configuration."
    else
        log "No LUKS root device detected; skipping TPM2 auto-unlock setup."
    fi
fi
