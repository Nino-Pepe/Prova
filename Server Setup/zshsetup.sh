...
install_dependencies() {
    ...
    if command -v apt-get >/dev/null 2>&1; then
        if [ -n "$sudo_cmd" ]; then
            ...
            $sudo_cmd apt-get install -y zsh git curl wget nano
        else
            apt-get update
            apt-get install -y zsh git curl wget nano
        fi
    elif command -v yum >/dev/null 2>&1; then
        if [ -n "$sudo_cmd" ]; then
            $sudo_cmd yum -y install zsh git curl wget nano
        else
            yum -y install zsh git curl wget nano
        fi
    elif command -v pacman >/dev/null 2>&1; then
        if [ -n "$sudo_cmd" ]; then
            $sudo_cmd pacman -Sy --noconfirm zsh git curl wget nano
        else
            pacman -Sy --noconfirm zsh git curl wget nano
        fi
    else
